...
def ask_goose_by_keywords(keywords):
    ...
    if 'text_text' not in collection.list_indexes():  # 'text_text' is the default name for a text index on the 'text' field
    ...
    ret = {}
    appended_posts = set() #make sure that each post can only be appended once for faster AI processing
    for k in keywords:
        ret[k] = [] #create
        query = {"$text": {"$search": k}}
...
def ask_sam_by_keywords(keywords):
    ...
    if 'content_text' not in collection.list_indexes():  # 'text_text' is the default name for a text index on the 'text' field
